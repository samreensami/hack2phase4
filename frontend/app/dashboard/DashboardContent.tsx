...
export default function DashboardContent({ initialStats }: { initialStats: Stats | null }) {
  ...
  useEffect(() => {
    // Only run client-side effects in browser
    ...
    // Use initial stats if available, otherwise fetch
    if (!initialStats) {
      fetchStats();
    } else {
      setLoading(false);
